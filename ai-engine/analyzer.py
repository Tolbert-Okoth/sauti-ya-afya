...
# ==============================================================================
# 🧠 PHYSICS ENGINE LOGIC (Your Core Algorithms)
# ==============================================================================
...
def analyze_audio(file_path, symptoms="", sensitivity_threshold=0.75):
    try:
        print(f"--- [START] Analysis Job (Hybrid V2 - Strict Normal) ---")
        ...
        for idx, chunk in enumerate(chunks):
            ...
            if ai_available and img:
                ...
                if is_vesicular:
                     ...
                     chunk_diagnosis = "Normal"; chunk_severity = 1; probs_list[-1] = torch.tensor([0.01, 0.98, 0.01]) 
                elif is_stable_normal:
                     print(f"   ✨ STABILITY CHECK: Non-impulsive (RMS={rms:.3f}). Forcing Normal.")
                     chunk_diagnosis = "Normal"; chunk_severity = 1; probs_list[-1] = torch.tensor([0.02, 0.96, 0.02]) 
                elif is_wheeze and not is_friction_noise:
                     print(f"   🌬️ WHEEZE DETECTED: Tonal Sound (Flux={flux:.2f}). Forcing Asthma.")
                     force_asthma = True; detected_wheezes += 1
                elif is_golden_normal and not is_coarse_crackle:
                     print(f"   ✨ GOLDEN ZONE: Physics matches Healthy Breath. Forcing Normal.")
                     chunk_diagnosis = "Normal"; chunk_severity = 1; probs_list[-1] = torch.tensor([0.05, 0.90, 0.05]) 
                ...
                if force_pneumonia:
                    chunk_diagnosis = "Pneumonia"; chunk_severity = 3; winner_prob = 0.90; probs_list[-1] = torch.tensor([0.05, 0.05, 0.90]); physics_override = True; pneumonia_chunks_detected += 1
                elif force_asthma:
                    chunk_diagnosis = "Asthma"; chunk_severity = 2; winner_prob = 0.85; probs_list[-1] = torch.tensor([0.85, 0.05, 0.10]); physics_override = True; asthma_chunks_detected += 1
                else:
                    if chunk_diagnosis == "Asthma":
                        if is_wheeze: asthma_chunks_detected += 1; chunk_severity = 2 
                        elif (transients > 4 or crest_factor > 15 or flux > 1.5) and centroid > 800 and not is_friction_noise:
                             print(f"   🛡️ VETO: AI=Asthma, but Sound is Chaotic. Overriding to Pneumonia.")
                             chunk_diagnosis = "Pneumonia"; chunk_severity = 3; winner_prob = 0.85; probs_list[-1] = torch.tensor([0.10, 0.05, 0.85]); physics_override = True; pneumonia_chunks_detected += 1
                        elif spectral_flatness > 0.35:
                             print(f"   ℹ️ INFO: AI=Asthma, but Sound is Flat. Likely Normal Breath."); chunk_diagnosis = "Normal"; chunk_severity = 1; winner_prob = 0.60; probs_list[-1] = torch.tensor([0.20, 0.60, 0.20]) 
                        else:
                             if chunk_diagnosis != "Normal": chunk_severity = 2 
                    elif chunk_diagnosis == "Pneumonia" and winner_prob > 0.60:
                        if spectral_flatness > 0.42:
                            print(f"   ℹ️ INFO: AI=Pneumonia, but Sound is Flat. Downgrading to Normal."); chunk_diagnosis = "Normal"; chunk_severity = 1; probs_list[-1] = torch.tensor([0.10, 0.80, 0.10])
                        else: pneumonia_chunks_detected += 1
                ...
                valid_chunks += 1
                if chunk_severity > highest_severity: highest_severity = chunk_severity
        ...
        if valid_chunks == 0:
            ...
            if probs_list:
                ...
                p_pneumonia = averaged_probs["Pneumonia"]; p_asthma = averaged_probs["Asthma"]
                if pneumonia_chunks_detected > asthma_chunks_detected:
                    final_diagnosis = "Pneumonia"; averaged_probs["Pneumonia"] = max(p_pneumonia, 0.80)
                elif asthma_chunks_detected > pneumonia_chunks_detected:
                    final_diagnosis = "Asthma"; averaged_probs["Asthma"] = max(p_asthma, 0.80)
                     ...
                     if abs(p_asthma - p_pneumonia) < 0.2:
                         ...
                         if detected_wheezes > 0: print("   -> Wheezes found. Winner: Asthma."); final_diagnosis = "Asthma"
                         elif total_transients > 5: print("   -> Transients found. Winner: Pneumonia."); final_diagnosis = "Pneumonia"
                         else: final_diagnosis = max(averaged_probs, key=averaged_probs.get)
                     else: final_diagnosis = max(averaged_probs, key=averaged_probs.get)
            ...
            if total_transients > 12 and avg_harm < 0.65 and avg_flat < 0.42 and final_diagnosis != "Pneumonia":
                 ...
                 final_diagnosis = "Pneumonia"; averaged_probs["Pneumonia"] = 0.85

        if symptoms:
            matched = []; risk_bonus = 0.0
            ...
            if risk_bonus > 0:
                ...
                new_winner = max(averaged_probs, key=averaged_probs.get)
                if averaged_probs[new_winner] > 0.60: highest_severity = max(highest_severity, SEVERITY_SCORE.get(new_winner, 0))
                if SEVERITY_SCORE.get(new_winner, 0) >= SEVERITY_SCORE.get(final_diagnosis.replace("Suspected ", ""), 0): final_diagnosis = new_winner
...
# ==============================================================================
# 🚀 FASTAPI SERVER ENTRY POINT (UPTIME ROBOT COMPATIBLE)
# ==============================================================================
...
# 🧠 IN-MEMORY JOB STORE (Replaces Redis for Lite usage)
job_store: Dict[str, dict] = {}

def process_analysis_task(ticket_id: str, file_path: str, symptoms: str):
    """Background worker that runs the physics engine"""
    print(f"👷 WORKER: Starting Job {ticket_id} (Symptoms: '{symptoms}')")
    try:
        results = analyze_audio(file_path, symptoms)
        job_store[ticket_id] = {"status": "completed", "result": results}
        # Clean up temp file
        if os.path.exists(file_path): os.remove(file_path)
    except Exception as e:
        job_store[ticket_id] = {"status": "failed", "error": str(e)}

@app.get("/")
def uptime_check():
    """
    🟢 UPTIME ROBOT TARGET
    This simple endpoint returns 200 OK.
    Configure Uptime Robot to ping: https://your-app-url.onrender.com/
    """
    return {"status": "alive", "service": "SautiYaAfya AI Engine"}

@app.post("/analyze")
async def start_analysis(background_tasks: BackgroundTasks, file: UploadFile = File(...), symptoms: Optional[str] = Form("")):
    """
    🎫 ISSUES TICKET
    Saves audio and spawns background worker. Returns Ticket ID immediately.
    """
    ticket_id = str(uuid.uuid4())
    temp_filename = f"temp_{ticket_id}.wav"
    
    with open(temp_filename, "wb") as buffer:
        buffer.write(await file.read())
    
    job_store[ticket_id] = {"status": "processing"}
    background_tasks.add_task(process_analysis_task, ticket_id, temp_filename, symptoms)
...
def check_status(ticket_id: str):
    """
    🔍 POLL STATUS
    React Frontend calls this every 2 seconds to check if job is done.
    """
    job = job_store.get(ticket_id)
    if not job:
