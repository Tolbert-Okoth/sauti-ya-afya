...
const Analytics = () => {
  ...
  if (!data) return <div className="text-center mt-5 text-white animate-pulse">Loading Epidemiology Data...</div>;
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className="glass-card p-2 border-0 shadow-sm text-white" style={{background: 'rgba(0,0,0,0.8)'}}>
          <p className="mb-0 fw-bold">{label ? `${label}` : payload[0].name}</p>
          <p className="mb-0 text-info fw-bold">{`Cases: ${payload[0].value}`}</p>
        </div>
      );
    }
    return null;
    ...
    <div className="container-fluid p-0">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
          <div className="d-flex align-items-center">
              <div className="bg-primary rounded-circle p-2 me-3 shadow-sm text-white">
                <FaChartPie size={20} />
              </div>
              <div>
                  <h3 className="fw-bold text-white mb-0">Epidemiology Analytics</h3>
                  <small className="text-white-50">Real-time surveillance data</small>
              </div>
          </div>
          
          <div className="glass-card px-4 py-2 text-center d-none d-md-block">
              <div className="small text-white-50 text-uppercase fw-bold">Total Screenings</div>
              <div className="h4 fw-bold text-white mb-0"><FaUsers className="me-2 opacity-50"/>{totalCases}</div>
          </div>
      </div>
      ...
      <div className="row g-4">
        ...
        <div className="col-md-6 col-lg-5">
            <div className="glass-card h-100 d-flex flex-column">
                <div className="d-flex justify-content-between align-items-center mb-3">
                    <h6 className="fw-bold text-white mb-0">Risk Distribution</h6>
                    <small className="text-white-50">By Severity</small>
                </div>
        ...
        <div className="col-md-6 col-lg-7">
            <div className="glass-card h-100 d-flex flex-column">
                <div className="d-flex justify-content-between align-items-center mb-3">
                    <h6 className="fw-bold text-white mb-0">Regional Hotspots</h6>
                    <FaMapMarkedAlt className="text-white-50 opacity-50"/>
                </div>
                <div style={{ height: '300px', width: '100%' }}>
                    <ResponsiveContainer>
                        <BarChart data={locData} margin={{top: 10, right: 30, left: 0, bottom: 0}}>
                            <XAxis dataKey="name" stroke="#b2bec3" fontSize={11} tickLine={false} axisLine={false} />
                            <YAxis allowDecimals={false} stroke="#b2bec3" fontSize={11} tickLine={false} axisLine={false}/>
                            <Tooltip content={<CustomTooltip />} cursor={{fill: 'rgba(255,255,255,0.05)'}} />
                            <Bar dataKey="cases" radius={[4, 4, 0, 0]} barSize={40}>
                                {locData.map((entry, index) => (
                                    <Cell key={`cell-${index}`} fill={index % 2 === 0 ? '#0984e3' : '#74b9ff'} />
                                ))}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
